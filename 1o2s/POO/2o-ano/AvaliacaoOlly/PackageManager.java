...
public class PackageManager {
    ...
    public void savePackages() {
        try (FileWriter writer = new FileWriter("Olly\\InformacaoFinal.txt")) {
            for (Package pack : packages) {
                writer.write(pack.getSender() + "; " + pack.getDestination() + "; " + pack.getWeight() + "; Erro");
            }
            System.out.println("Packages saved successfully.");
        } catch (IOException e) {
            System.out.println("An error occurred while saving packages: " + e.getMessage());
        }
    }
}
